...
class EditAsiento extends ExtendedController\PanelController
{
    ...
    */
    ...
    private function replaceConcept($results)
    {
        ...
        $search[]='%document%';$replace[]= $accounting->documento;
        $search[]='%date%' ; $replace[]= date('d-m-Y');
        $search[]='%date-entry%' ;$replace[]= $accounting->fecha;
        $search[]='%month%' ;$replace[]= $this->i18n->trans(date('F', strtotime($accounting->fecha)));
