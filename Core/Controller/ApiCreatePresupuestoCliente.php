...
class ApiCreatePresupuestoCliente extends ApiController
{
    protected function runResource(): void
    {
        ...
        if ($codalmacen && false === $presupuesto->setWarehouse($codalmacen)) {
            $this->response->setStatusCode(Response::HTTP_NOT_FOUND);
            $this->response->setContent(json_encode([
                'status' => 'error',
        ...
        $hora = $this->request->get('hora', $presupuesto->hora);
        if ($fecha && false === $presupuesto->setDate($fecha, $hora)) {
            $this->response->setStatusCode(Response::HTTP_BAD_REQUEST);
            $this->response->setContent(json_encode([
                'status' => 'error',
        ...
        if ($coddivisa) {
            $presupuesto->setCurrency($coddivisa);
        }

        // asignamos el resto de campos del modelo
        foreach ($presupuesto->getModelFields() as $key => $field) {
            if ($this->request->request->has($key)) {
                $presupuesto->{$key} = $this->request->request->get($key);
            }
        }
        ...
        if (false === $this->saveLines($presupuesto)) {
            $db->rollback();
            return;
        }
        ...
        $this->response->setContent(json_encode([
            'doc' => $presupuesto->toArray(),
            'lines' => $presupuesto->getLines(),
        ]));
    }

    protected function saveLines(PresupuestoCliente &$presupuesto): bool
    {
        ...
        foreach ($lineas as $line) {
            $newLine = empty($line['referencia'] ?? '') ?
                $presupuesto->getNewLine() :
                $presupuesto->getNewProductLine($line['referencia']);
        ...
        if (false === Calculator::calculate($presupuesto, $newLines, true)) {
            $this->response->setStatusCode(Response::HTTP_INTERNAL_SERVER_ERROR);
            $this->response->setContent(json_encode([
                'status' => 'error',
