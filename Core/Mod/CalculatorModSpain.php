...
class CalculatorModSpain implements CalculatorModInterface
{
    ...
    public function getSubtotals(array &$subtotals, BusinessDocument $doc, array $lines): bool
    {
        ...
        foreach ($lines as $line) {
            ...
            if (!empty($line->excepcioniva) && $line->excepcioniva !== TaxException::ES_TAX_EXCEPTION_22) $allE3 = false;
            if (!empty($line->excepcioniva) && $line->excepcioniva !== TaxException::ES_TAX_EXCEPTION_23_24) $allE4 = false;
            if (!empty($line->excepcioniva) && $line->excepcioniva !== TaxException::ES_TAX_EXCEPTION_21) $allE2 = false;
            if (!empty($line->excepcioniva) && $line->excepcioniva !== TaxException::ES_TAX_EXCEPTION_25) $allE5 = false;
    ...
    {
        if (isset($exenciones[TaxException::ES_TAX_EXCEPTION_20]) && (count($exenciones) > 1 || $hasIva)) {
            Tools::log()->warning('Excepción fiscal ES_20 no puede combinarse con IVA o con otras exenciones.');
            return false;
        }

        if (isset($exenciones[TaxException::ES_TAX_EXCEPTION_21]) && (count($exenciones) > 1 || $hasIva)) {
            Tools::log()->warning('Excepción fiscal ES_21 no puede combinarse con IVA o con otras exenciones.');
            return false;
        }

        if ((isset($exenciones[TaxException::ES_TAX_EXCEPTION_22]) || isset($exenciones[TaxException::ES_TAX_EXCEPTION_23_24]))
            && ($hasIva || isset($exenciones[TaxException::ES_TAX_EXCEPTION_21]) || isset($exenciones[TaxException::ES_TAX_EXCEPTION_20]))) {
            Tools::log()->warning('Excepción fiscal ES_22 o ES_23_24 no puede combinarse con IVA, ES_21 o ES_20.');
            return false;
        }

        if ((isset($exenciones[TaxException::ES_TAX_EXCEPTION_7]) || isset($exenciones[TaxException::ES_TAX_EXCEPTION_14])) && $hasIva) {
    ...
    private function validateGlobalExemption(?string $globalEx, bool $allZeroIva, array $exenciones): bool
    {
        ...
        if ($globalEx === InvoiceOperation::ES_INTRA_COMMUNITY) {
            if (!$allZeroIva || (count($exenciones) && !isset($exenciones[TaxException::ES_TAX_EXCEPTION_22])
                    && !isset($exenciones[TaxException::ES_TAX_EXCEPTION_23_24]) && !isset($exenciones[TaxException::ES_TAX_EXCEPTION_25]))) {
        ...
        if ($globalEx === InvoiceOperation::ES_EXPORT) {
            if (!$allZeroIva || (count($exenciones) && !isset($exenciones[TaxException::ES_TAX_EXCEPTION_21]))) {
    ...
    private function validateLineExemptions(BusinessDocument $doc, BusinessDocumentLine $line, ?string $subjectFiscalID, Contacto $addressShipping, ?string $globalEx): bool
    {
        ...
        if ($line->excepcioniva === TaxException::ES_TAX_EXCEPTION_20 && $line->iva > 0) {
            Tools::log()->warning('Excepción fiscal "' . $exceptions['ES_20'] . '" no puede tener IVA.');
            return false;
        }

        // E2: ES_21 solo si cliente fuera UE
        if ($line->excepcioniva === TaxException::ES_TAX_EXCEPTION_21 && $line->iva > 0 && Paises::miembroUE($doc->codpais)) {
        ...
        if ($line->excepcioniva === TaxException::ES_TAX_EXCEPTION_22 && $line->iva > 0 && !Paises::miembroUE($doc->codpais)
        ...
        if ($line->excepcioniva === TaxException::ES_TAX_EXCEPTION_23_24 && $line->iva > 0 && !Paises::miembroUE($doc->codpais)
        ...
        if ($line->excepcioniva === TaxException::ES_TAX_EXCEPTION_25 && $line->iva > 0 && !empty($addressShipping->id()) && $addressShipping->codpais === 'ESP') {
        ...
        if ($line->excepcioniva === TaxException::ES_TAX_EXCEPTION_OTHER && $line->iva > 0) {
        ...
        if (in_array($line->excepcioniva, [TaxException::ES_TAX_EXCEPTION_84, TaxException::ES_TAX_EXCEPTION_7, TaxException::ES_TAX_EXCEPTION_14, TaxException::ES_TAX_EXCEPTION_68_70])
            && $line->iva > 0) {
        ...
        if ($globalEx === InvoiceOperation::ES_INTRA_COMMUNITY) {
            ...
            if ($doc->subjectColumn() === 'codcliente' && !in_array($line->excepcioniva, [TaxException::ES_TAX_EXCEPTION_22, TaxException::ES_TAX_EXCEPTION_23_24, TaxException::ES_TAX_EXCEPTION_25])) {
            ...
            if ($doc->subjectColumn() === 'codproveedor' &&
                ...
                !in_array($line->excepcioniva, [TaxException::ES_TAX_EXCEPTION_84, TaxException::ES_TAX_EXCEPTION_14, TaxException::ES_TAX_EXCEPTION_68_70])) {
        ...
        if ($globalEx === InvoiceOperation::ES_EXPORT && $line->excepcioniva !== TaxException::ES_TAX_EXCEPTION_21) {
