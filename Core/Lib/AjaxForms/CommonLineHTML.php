...
trait CommonLineHTML
{
    ...
    private static function genericBool(string $idlinea, BusinessDocumentLine $line, TransformerDocument $model, string $field, string $label): string
    {
        ...
        $options = $line->{$field} ?
            ['<option value="0">' . Tools::trans('no') . '</option>', '<option value="1" selected>' . Tools::trans('yes') . '</option>'] :
            ['<option value="0" selected>' . Tools::trans('no') . '</option>', '<option value="1">' . Tools::trans('yes') . '</option>'];
        return '<div class="col-6">'
    ...
    private static function titleTotal(): string
    {
        ...
        return '<div class="col-lg-1 text-end order-7 columSubtotal ' . $cssSubtotal . '">' . Tools::trans('subtotal') . '</div>'
            . '<div class="col-lg-1 text-end order-7 columNeto ' . $cssNeto . '">' . Tools::trans('net') . '</div>';
    }
}
