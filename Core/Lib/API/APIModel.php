...
class APIModel extends APIResourceClass
{
    ...
    private function pluralize($text): string
    {
        ...
        if (substr($text, -1) === 's') {
            $plural = strtolower($text);
        } elseif (substr($text, -3) === 'ser' || substr($text, -4) === 'tion') {
            $plural = strtolower($text) . 's';
        } elseif (\in_array(substr($text, -1), ['a', 'e', 'i', 'o', 'u', 'k'], false)) {
            $plural = strtolower($text) . 's';
        } else {
            $plural = strtolower($text) . 'es';
        }
    ...
    public function doPOST(array $params): bool
    {
        ...
        $this->model->loadFromCode($params[0]);
        $this->model->{$cod} = $params[0];

        // Retrieve the past data, and replace the changes
        $values = $this->request->request->all();
        if (isset($values[$cod]) && $values[$cod] != $params[0]) {
            $this->setError("Can't change the key. Key '$cod'' changed from {$params[0]} to {$values[$cod]}.", $values);
            return false;
        }
    ...
    {
        if ($this->model->loadFromCode($params[0])) {
            $data = (array)$this->model;
            $this->model->delete();
            $this->setOk('record-deleted', (array)$data);
            return true;
        }

        $this->setError('data-not-found', $params[0] . ' not found in ' . $this->singularName);
        return false;
    }
    ...
    public function processResource(string $name, array $params): bool
    {
        try {
            ...
            return parent::processResource($name, $params);
        } catch (\Exception $ex) {
