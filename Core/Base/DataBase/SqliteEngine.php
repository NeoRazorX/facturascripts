    ...
    public function connect(&$error)
    {
        ...
        try {
            if (class_exists('Pdo\Sqlite')) {
                $result = new \Pdo\Sqlite('sqlite:' . $database);
            } else {
            ...
            if (method_exists($result, 'createFunction')) {
                $result->createFunction('regexp', $regexpCallback, 2);
            } else {
                $result->sqliteCreateFunction('regexp', $regexpCallback, 2);
            }
