...
class Divisa extends Base\ModelClass
{
    ...
    public function test()
    {
        ...
        if (!preg_match('/^[A-Z0-9]{1,3}$/i', $this->coddivisa)) {
            self::$miniLog->warning(self::$i18n->trans('invalid-alphanumeric-code', ['%value%' => $this->coddivisa, '%column%' => 'coddivisa', '%min%' => '1', '%max%' => '3']));
        } elseif ($this->codiso !== null && !preg_match('/^[A-Z0-9]{1,5}$/i', $this->codiso)) {
            self::$miniLog->warning(self::$i18n->trans('invalid-alphanumeric-code', ['%value%' => $this->codiso, '%column%' => 'codiso', '%min%' => '1', '%max%' => '5']));
