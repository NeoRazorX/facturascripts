...
class general_pedidos_prov extends fs_controller
{
   ...
   {
      parent::__construct('general_pedidos_prov', 'Pedidos de proveedor', 'compras', FALSE, TRUE, TRUE);
   }
   ...
   protected function process()
   {
      ...
      else
      {
         ...
         $this->buttons[] = new fs_button_img('b_nuevo_pedido', 'Nuevo', 'add.png', 'index.php?page=general_nuevo_pedido');
         $this->buttons[] = new fs_button('b_agrupar_pedidos', 'Agrupar', 'index.php?page=general_agrupar_pedidos_pro');
   ...
   public function buscar_lineas()
   {
      ...
      $this->template = 'ajax/general_lineas_pedidos_prov';
   ...
   {
      $pedi1 = new pedido_proveedor();
      $pedi1 = $pedi1->get($_POST['delete']);
      if($pedi1)
      {
         ...
         if( isset($_POST['stock']) )
         {
            ...
            foreach($pedi1->get_lineas() as $linea)
            {
               $art0 = $articulo->get($linea->referencia);
               if($art0)
               {
                  $art0->sum_stock($pedi1->codalmacen, 0 - $linea->cantidad);
                  $art0->save();
               }
            }
         }
         ...
         if( $pedi1->delete() )
            $this->new_message("Pedido ".$pedi1->codigo." borrado correctamente.");
         else
