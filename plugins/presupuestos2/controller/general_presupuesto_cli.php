...
class general_presupuesto_cli extends fs_controller
{
   ...
   {
      $cliente = $this->cliente->get($pedido->codcliente);
   ...
   private function generar_pdf_simple($archivo = FALSE)
   {
      ...
      if($lineas)
      {
         ...
         while( $linea_actual < count($lineas) )
         {
            ...
            else if($linea_actual >= $lineaspedi)
               $pdf_doc->pdf->ezText($salto, 11);
            else
               $pdf_doc->pdf->ezText("\n", 11);
            ...
            $pdf_doc->pdf->addText(10, 10, 8, $pdf_doc->center_text($this->empresa->pie_pedido, 153), 0, 1.5);
