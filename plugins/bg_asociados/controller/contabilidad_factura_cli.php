      ...
      if( $lineas )
      {
         ...
         while($linea_actual < $lineasfact)
         {
            ...
            else /// esta es la cabecera de la página para los modelos 'simple' y 'firma'
            {
               ...
               $pdf_doc->pdf->addText(30, 770, 16, 'Factura: '.$this->factura->codigo);
               
               $pdf_doc->new_table();
               $pdf_doc->add_table_row(
                  array(
                     'dato1' => "Fecha de factura: ".$this->factura->fecha."\nExp. BG. Número: ".$this->cliente->expbgnum,
                     'dato2' => $this->show_precio($this->factura->total, $this->factura->coddivisa)
                  )
               );
               $pdf_doc->save_table(
                  array(
               ...
               $pdf_doc->add_table_row(
                  array(
                     ...
                     'dato2' => 'Nie/Nif: '.$this->factura->cifnif
                  )
               );
               $pdf_doc->add_table_row(
                  array(
                     'dato1' => $this->empresa->direccion,
                     'dato2' => $this->factura->direccion
                  )
               );
               $pdf_doc->add_table_row(
                  array(
                     'dato1' => 'CP: '.$this->empresa->codpostal,
                     'dato2' => 'CP: '.$this->factura->codpostal
                  )
               );
               $pdf_doc->add_table_row(
                  array(
                     'dato1' => $this->empresa->ciudad,
                     'dato2' => $this->factura->ciudad
                  )
               );
               $pdf_doc->add_table_row(
                  array(
            ...
            for($i = $linea_actual; (($linea_actual < ($lppag + $i)) AND ($linea_actual < $lineasfact));)
            {
               ...
               $fila = array(
                  ...
                  'subtotal' => $this->show_precio($subtotal, $this->factura->coddivisa)
               );
