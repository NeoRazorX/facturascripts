...
class ventas_pedido extends fs_controller
{
    ...
    {
        parent::__construct( __CLASS__, ucfirst( FS_PEDIDO ), 'ventas', FALSE,
                                                 FALSE );
    }
    ...
    protected function process()
    {
        ...
        if ( $this->pedido )
        {
            ...
            {
                $this->pedido->status = $_REQUEST[ 'status' ];
                if ( $this->pedido->save() )
                {
    ...
    private function modificar()
    {
        ...
        if ( is_null( $this->pedido->idalbaran ) )
        {
            ...
            if ( $eje0 )
            {
                $this->pedido->fecha = $eje0->get_best_fecha( $_POST[ 'fecha' ],
                                                              TRUE );
            ...
            if ( isset( $_POST[ 'numlineas' ] ) )
            {
                ...
                $this->pedido->totaliva     = round( $this->pedido->totaliva,
                                                     FS_NF0 );
                $this->pedido->totalirpf    = round( $this->pedido->totalirpf,
                                                     FS_NF0 );
                $this->pedido->totalrecargo = round( $this->pedido->totalrecargo,
                                                     FS_NF0 );
        ...
        if ( $this->pedido->save() )
        {
            ...
            $this->new_change( ucfirst( FS_PEDIDO ) . ' Cliente ' . $this->pedido->codigo,
                                        $this->pedido->url() );
        }
        else
    ...
    private function generar_albaran()
    {
        ...
        else if ( $albaran->save() )
        {
            ...
            foreach ( $this->pedido->get_lineas() as $l )
            {
                ...
                if ( $n->save() )
                {
                    ...
                    if ( !is_null( $n->referencia ) )
                    {
                        ...
                        $articulo->sum_stock( $albaran->codalmacen,
                                              0 - $l->cantidad );
                    }
                }
                else
            ...
            if ( $continuar )
            {
                ...
                // Si tenemos albaran, el status es 1 (aprobado)
