...
class ventas_presupuesto extends fs_controller
{
    ...
    {
        parent::__construct( __CLASS__, ucfirst( FS_PRESUPUESTO ), 'ventas',
                                                 FALSE, FALSE );
    }
    ...
    protected function process()
    {
        ...
        if ( $this->presupuesto )
        {
            ...
            {
                $this->presupuesto->status = $_REQUEST[ 'status' ];
    ...
    private function modificar()
    {
        ...
        if ( is_null( $this->presupuesto->idpedido ) )
        {
            ...
            if ( $eje0 )
            {
                $this->presupuesto->fecha     = $eje0->get_best_fecha( $_POST[ 'fecha' ],
                                                                       TRUE );
            ...
            if ( isset( $_POST[ 'numlineas' ] ) )
            {
                ...
                /// redondeamos
                $this->presupuesto->neto         = round( $this->presupuesto->neto,
                                                          FS_NF0 );
                $this->presupuesto->totaliva     = round( $this->presupuesto->totaliva,
                                                          FS_NF0 );
                $this->presupuesto->totalirpf    = round( $this->presupuesto->totalirpf,
                                                          FS_NF0 );
                $this->presupuesto->totalrecargo = round( $this->presupuesto->totalrecargo,
                                                          FS_NF0 );
        ...
        if ( $this->presupuesto->save() )
        {
            ...
            $this->new_change( ucfirst( FS_PRESUPUESTO ) . ' Cliente ' . $this->presupuesto->codigo,
                                        $this->presupuesto->url() );
        }
        else
    ...
    private function generar_pedido()
    {
        ...
        else if ( $pedido->save() )
        {
            ...
            if ( $continuar )
            {
                ...
                // Si tenemos pedido, el status es 1 (aprobado)
