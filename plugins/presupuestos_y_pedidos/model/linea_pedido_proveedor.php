...
/**
 * Línea de un pedido de proveedor.
 */
class linea_pedido_proveedor extends fs_model
{
   ...
   public function test()
   {
      ...
      if( !$this->floatcmp($this->pvptotal, $total, 2, TRUE) )
      {
         $this->new_error_msg("Error en el valor de pvptotal de la línea ".$this->referencia.
            " del pedido. Valor correcto: ".$total);
         return FALSE;
      }
      else if( !$this->floatcmp($this->pvpsindto, $totalsindto, 2, TRUE) )
      {
         $this->new_error_msg("Error en el valor de pvpsindto de la línea ".$this->referencia.
            " del pedido. Valor correcto: ".$totalsindto);
         return FALSE;
      }
      else
         return TRUE;
   ...
   public function search_from_proveedor($codproveedor, $query='', $offset=0)
   {
      ...
      $sql = "SELECT * FROM ".$this->table_name." WHERE idpedido IN
         (SELECT idpedido FROM pedidosprov WHERE codproveedor = ".$this->var2str($codproveedor).") AND ";
      if( is_numeric($query) )
      {
         $sql .= "(referencia LIKE '%".$query."%' OR descripcion LIKE '%".$query."%')";
      }
