...
class presupuesto_cliente extends fs_model
{
    ...
    public function __construct( $p = FALSE )
    {
        ...
        else
        {
            ...
            $this->finoferta     = date( "d-m-Y",
                                         strtotime( Date( 'd-m-Y' ) . " +30 days" ) );
    ...
    {
        $sec = new secuencia();
        $sec = $sec->get_by_params2( $this->codejercicio, $this->codserie,
                                     'npresupuestocli' );
        if ( $sec )
        {
        ...
        $this->codigo = $this->codejercicio . sprintf( '%02s', $this->codserie ) . sprintf( '%06s',
                                                                                            $this->numero );
    }
    ...
    public function full_test( $duplicados = TRUE )
    {
        ...
        else if ( !$this->floatcmp( $this->totaleuros,
                                    $this->total * $this->tasaconv, FS_NF0, TRUE ) )
        {
    ...
    public function save()
    {
        if ( $this->test() )
        {
            if ( $this->exists() )
            {
               ...
               observaciones = " . $this->var2str( $this->observaciones ) . ", status = " . $this->var2str( $this->status ) . ", porcomision = " . $this->var2str( $this->porcomision ) . ",
            ...
            else
            {
               ...
               " . $this->var2str( $this->observaciones ) . "," . $this->var2str( $this->status ) . "," . $this->var2str( $this->porcomision ) . "," . $this->var2str( $this->provincia ) . ",
    ...
    {
        $preslist     = array ();
        $presupuestos = $this->db->select_limit( "SELECT * FROM " . $this->table_name . " ORDER BY fecha DESC, codigo DESC",
    ...
    public function all_ptepedir( $offset = 0, $order = 'DESC' )
    {
        ...
        $presupuestos = $this->db->select_limit( "SELECT * FROM " . $this->table_name .
                " WHERE idpedido IS NULL AND status=0 ORDER BY fecha " . $order . ", codigo " . $order,
                                                 FS_ITEM_LIMIT, $offset );
        if ( $presupuestos )
        {
            foreach ( $presupuestos as $p )
    ...
    public function all_rechazados( $offset = 0, $order = 'DESC' )
    {
        $preclist     = array ();
        $presupuestos = $this->db->select_limit( "SELECT * FROM " . $this->table_name .
                " WHERE status=2 ORDER BY fecha " . $order . ", codigo " . $order,
                                                 FS_ITEM_LIMIT, $offset );
        if ( $presupuestos )
        {
            foreach ( $presupuestos as $p )
    ...
    public function search( $query, $offset = 0 )
    {
        ...
        else if ( preg_match( '/^([0-9]{1,2})-([0-9]{1,2})-([0-9]{4})$/i',
                              $query ) ) /// es una fecha
        {
        ...
        else
        {
            $consulta .= "lower(codigo) LIKE '%" . $query . "%' OR lower(numero2) LIKE '%" . $query . "%' "
                    . "OR lower(observaciones) LIKE '%" . str_replace( ' ', '%',
                                                                       $query ) . "%'";
        }
        $consulta .= " ORDER BY fecha DESC, codigo DESC";

        $presupuestos = $this->db->select_limit( $consulta, FS_ITEM_LIMIT,
                                                 $offset );
        if ( $presupuestos )
        {
            foreach ( $presupuestos as $p )
