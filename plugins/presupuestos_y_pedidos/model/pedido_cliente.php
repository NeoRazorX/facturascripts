...
class pedido_cliente extends fs_model
{
    ...
    public function __construct( $p = FALSE )
    {
        ...
        if ( $p )
        {
            ...
            if ( isset( $p[ 'fechasalida' ] ) )
                $this->fechasalida = Date( 'd-m-Y',
                                           strtotime( $p[ 'fechasalida' ] ) );
        }
        else
        {
            $this->idpedido      = NULL;
    ...
    {
        $pedilist = array ();
        $pedidos  = $this->db->select_limit( "SELECT * FROM " . $this->table_name . " ORDER BY fecha DESC, codigo DESC",
    ...
    public function all_ptealbaran( $offset = 0, $order = 'DESC' )
    {
        ...
        $pedidos  = $this->db->select_limit( "SELECT * FROM " . $this->table_name .
                " WHERE idalbaran IS NULL AND status=0 ORDER BY fecha " . $order . ", codigo " . $order,
                                             FS_ITEM_LIMIT, $offset );
        if ( $pedidos )
        {
            foreach ( $pedidos as $p )
