...
function recalcular()
{
   ...
   for(var i=1; i<=numlineas; i++)
   {
      if($("#linea_"+i).length > 0)
      {
         ...
         $("#total_"+i).val( l_uds*l_pvp);
         $("#totiva_"+i).val( fs_round(l_uds*l_pvp*(100+l_iva)/100, fs_nf0) );
   ...
   for(var i=1; i<=numlineas; i++)
   {
      if($("#linea_"+i).length > 0)
      {
         ...
         if( isNaN(l_totiva) )
            ...
            l_dto = 0;
            l_pvp = 10000*l_totiva/(l_uds*100*(100+l_iva));
            if( isNaN(l_pvp) )
               l_pvp = 0;
...
function add_cliente(codpostal,ciudad,provincia,pais,direccion,telefono,cifnif,nombre)
{
         ...
         $.post(tpv_url,parametros,function(respuesta){
               $("#selectdinamico").html(respuesta);
...
function add_articulo(ref,desc,pvp,dto,iva)
{
   ...
   $("#lineas_albaran").prepend("<tr id=\"linea_"+numlineas+"\">\n\
         <td><input type=\"hidden\" name=\"referencia_"+numlineas+"\" size=\"3\"  value=\""+ref+"\"/>\n\
         ...
         <td><input type=\"number\" step=\"any\" id=\"cantidad_"+numlineas+"\" size=\"3\"  class=\"form-control text-right\" name=\"cantidad_"+numlineas+
         ...
         <td><input type=\"text\" size=\"5\" class=\"form-control text-right\" id=\"pvp_"+numlineas+"\" name=\"pvp_"+numlineas+"\" value=\""+pvp+
            "\" onkeyup=\"recalcular()\" onclick=\"this.select()\" autocomplete=\"off\"/></td>\n\
         <td><input type=\"text\" class=\"form-control text-right\" size=\"5\"  id=\"total_"+numlineas+"\" name=\"total_"+numlineas+
            "\" readonly/></td>\n\
         <td class=\"text-right\"><div class=\"form-control\">"+iva+"</div></td>\n\
         <td><input type=\"text\" class=\"form-control text-right\" size=\"5\"  id=\"totiva_"+numlineas+"\" name=\"totiva_"+numlineas+
